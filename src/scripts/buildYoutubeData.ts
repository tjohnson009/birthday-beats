...
export const getYoutubeVideoInfo = async (title: string, artist: string) => {
    ...
    if (!response.ok) {
        throw new Error(
            `Failed to fetch Youtube music video for ${title} by ${artist}: ${response.status} ${await response.text()}`,
        );
    }
...
for (let numberOneSong of sortedNumberOnes) {
    if (`${numberOneSong.song}|${numberOneSong.artist}` in videosManifest) {
        ...
        if (youtubeVideoInfo) {
            console.log(
                `[${count}] ${numberOneSong.song} — ${numberOneSong.artist} → ${youtubeVideoInfo.videoId} (${youtubeVideoInfo.title} / ${youtubeVideoInfo.channelName})`,
            );
        } else {
            noMatchCount++;
...
console.log(
    `Searched ${count} songs (${noMatchCount} no match). Manifest now covers ${Object.keys(videosManifest).length} of ${totalSongs} songs.`,
);
